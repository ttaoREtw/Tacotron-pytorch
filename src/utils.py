...
def save_states(global_step, mel_outputs, linear_outputs, attn, y,
                input_lengths, checkpoint_dir, audio_processor):
    ...
    save_spectrogram(path, linear_output, audio_processor)

    # Predicted audio signal
    signal = audio_processor.inv_spectrogram(linear_output.T)
    path = join(checkpoint_dir, "step{}_predicted.wav".format(
        global_step))
    audio_processor.save_wav(signal, path)
...
def save_spectrogram(path, linear_output, audio_processor):
    spectrogram = audio_processor._denormalize(linear_output)
